...
- Pulls Istio request/error and latency/bytes percentiles (from histograms)
...
- Output CSV columns are transformed to match "simple_data.csv" EXACTLY (names + order),
  while keeping the rest of the script behavior (args, printing, etc.) the same.
"""
...
def _parse_args():
    ...
    ap.add_argument("--namespace", default="sock-shop", help="K8s namespace for service label selectors")
    ...
    ap.add_argument("--step", default="15s", help="Query step (e.g. 5s, 15s, 1m)")
    ap.add_argument("--inject", type=int, help="Injection epoch seconds (optional shortcut to set start/end)")
    ap.add_argument("--window-minutes", type=int, default=10, help="Window minutes on each side of --inject")
    ap.add_argument("--out", default="merged_v4.csv", help="Output CSV path")
    ap.add_argument("--timeout", type=int, default=120, help="HTTP timeout seconds")
    ap.add_argument("--verify-tls", action="store_true", help="Verify TLS certs to Prometheus")
    ap.add_argument("--lat-histogram", choices=["ms","s"], default="ms",
                   help="Istio duration unit in histogram name: 'ms' for istio_request_duration_milliseconds_bucket, 's' for *_seconds_bucket")
    ap.add_argument("--bytes-histogram", choices=["response","request"], default="response",
                   help="Which bytes histogram to use for percentile: response_bytes_bucket or request_bytes_bucket")
...
# -----------------------------
# PromQL builders (keep same semantics)
# -----------------------------

def q_container_cpu_usage(namespace: str, svc: str) -> str:
    return f'''
sum by (pod) (
  rate(container_cpu_usage_seconds_total{{namespace="{namespace}", pod=~"{svc}.*", image!=""}}[{rate_window}])
)
'''.strip()
...
def q_mem_usage(namespace: str, svc: str) -> str:
    return f'''
sum by (pod) (
  container_memory_usage_bytes{{namespace="{namespace}", pod=~"{svc}.*", image!=""}}
)
'''.strip()

def _istio_latency_quantile(namespace: str, svc: str, qtile: float, unit: str) -> str:
...
histogram_quantile({qtile},
  sum by (le) (
    rate({metric}{{reporter="source", destination_workload="{svc}", destination_workload_namespace="{namespace}"}}[{rate_window}])
  )
)
'''.strip()

def q_istio_requests(namespace: str, svc: str) -> str:
    return f'''
sum (
  rate(istio_requests_total{{reporter="source", destination_workload="{svc}", destination_workload_namespace="{namespace}"}}[{rate_window}])
)
'''.strip()

def q_istio_errors(namespace: str, svc: str) -> str:
    return f'''
sum (
  rate(istio_requests_total{{reporter="source", destination_workload="{svc}", destination_workload_namespace="{namespace}", response_code=~"5.."}}[{rate_window}])
)
'''.strip()

# --- Node metrics (left intact, but will be omitted from final CSV to match simple_data schema) ---
...
# -----------------------------
# SIMPLE schema (exactly match simple_data.csv)
# -----------------------------
...
def collect_for_service_simple(prom, ns, svc, start, end, step, timeout, verify, lat_unit) -> pd.DataFrame:
    """
    Collect only the metrics needed for simple_data.csv:
      - <svc>_cpu  from container_cpu_usage_seconds_total rate
      - <svc>_mem  from container_memory_usage_bytes
      - <svc>_workload from istio_requests_total rate (subset)
      - <svc>_error    from istio 5xx rate (subset)
      - <svc>_latency-50/-90 from istio duration histogram quantiles (subset)
    """
    df_all = None
...
def main():
    ...
    # Nodes (optional) - still collected to keep functionality, but will be omitted from final CSV
    if args.nodes:
    ...
    # Convert time index to epoch seconds (same as your original)
    merged.index = merged.index.astype("int64") // 10**9
    merged.index.name = "time"

    # --- Force output schema to match simple_data.csv exactly (names + order) ---
    # Start with only the desired columns; create missing ones as NaN; drop extras (including node_*).
    out_df = merged.copy()
    ...
    out_df = out_df.reset_index()  # make "time" a column
    out_df = out_df[SIMPLE_COLS_ORDER]  # exact order, drop all other columns
